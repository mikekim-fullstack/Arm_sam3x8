...
{
    memset((void*)rx_buffer.buffer,0, sizeof(uint8_t)*UART_BUFFER_SIZE);
    rx_buffer.head=0;
    rx_buffer.tail=0;

    memset((void*)tx_buffer.buffer,0, sizeof(uint8_t)*UART_BUFFER_SIZE);
    tx_buffer.head=0;
    tx_buffer.tail=0;
...
void MKSerial::flush()
{
  rx_buffer.head = rx_buffer.tail;//Before
