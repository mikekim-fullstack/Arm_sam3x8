...
volatile uint8_t endstops[3] = {0, 0, 0};
...
static void startTimer3(uint32_t frequency = 10)
{
  Tc *tc = TC1;
  uint32_t channel = 0;
...
static void startTimer4(uint32_t frequency = 10)
{
  Tc *tc = TC1;
  uint32_t channel = 1;
...
inline static void setMotorDirection(int8_t dir)
{

  if (dir == 1)
  { // CCW
   pioWriteOutput(IOMap[ST_DIR].pIO, IOMap[ST_DIR].pin, HIGH);
  }
  else
  { // CW
    pioWriteOutput(IOMap[ST_DIR].pIO, IOMap[ST_DIR].pin, LOW);
  }
}
...
void PIOB_Handler()
{
  if (!irqBBusy)
  {
    ...
    if ((isr & IOMap[HOMESTOP].pin)) // & IOMap[HOMESTOP].pin)
    {
      irqBBusy = 1;
...
{
  TC_GetStatus(TC0, 1);
  if (speedData[nTimer].step_count < speedData[nTimer].steps){
    posData.abs_step_pos += speedData[nTimer].dir;
    ++speedData[nTimer].step_count;
    writePluse(IOMap[ST_PULSE].pIO, IOMap[ST_PULSE].pin, LOW);
...
inline void reportStatus()
{
   sprintf(tmpBuffer, "R%d P%d O%d S%d H%d",posData.CMDCode, (int32_t)posData.abs_step_pos, 
       OperationMode, endstops[0], pioReadInput(IOMap[HOMESTOP].pIO, IOMap[HOMESTOP].pin));
      mkSerial.println(tmpBuffer); //for serial notification
}
void reportStatus(int codeValue)
{
   sprintf(tmpBuffer, "R%d P%d O%d S%d H%d",codeValue, (int32_t)posData.abs_step_pos,
       OperationMode, endstops[0], pioReadInput(IOMap[HOMESTOP].pIO, IOMap[HOMESTOP].pin));
      mkSerial.println(tmpBuffer); //for serial notification
}
/////////////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////
  ...
  while (1)
  {
    ...
    if (gIRQ_TC_FLAG_DONE[nTimer] == 1)
    {
      ...
      mkSerial.print("CNT:");
      mkSerial.println(speedData[nTimer].step_count); 
      ...
      // ...NOTIFY TO HOST...
      if (OperationMode == HOMING && endstops[0]==2)
    ...
    {
      // +++ HIT HOME POSITION +++ //
      uint8_t statusSW = pioReadInput(IOMap[HOMESTOP].pIO, IOMap[HOMESTOP].pin); 
      if (statusSW)
      {
        ...
        if(endstops[0] == 0 ){
           ...
           // OperationMode = JOB_DONE;
            ...
            sprintf(tmpBuffer, "R28 P%d O%d S%d H%d", posData.abs_step_pos, OperationMode, 1, statusSW);
            mkSerial.println(tmpBuffer);
          }
      ...
      else 
      {
        if(endstops[0] == 1){
           ...
           // OperationMode = JOB_DONE;
            posData.abs_step_pos=0;
            //speedData[nTimer].step_count = 0;
            sprintf(tmpBuffer, "R28 P%d O%d S%d H%d", posData.abs_step_pos, OperationMode, 2, statusSW);
            mkSerial.println(tmpBuffer);
    ...
    if (!pioReadInput(IOMap[ENDSTOP].pIO, IOMap[ENDSTOP].pin))
    {
      speedData[nTimer].activated = false;
      stopTimer1();
