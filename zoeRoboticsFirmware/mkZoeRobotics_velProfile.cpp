...
// Linear for index 0
  ...
  for(int n=0; n<maxN; n++)
  {
    ...
    // Handling Acceleration and Deceleration by adding more time
     if(n<acc_dec_area)                   deltaT = deltaT_orignal*exp(0.2);
     else if(n> (maxN-acc_dec_area)) deltaT = deltaT_orignal*exp(0.2);
    ...
    //% it to next step...
      ...
      if (steps[CH] == 0) {
        ...
        if(n==0) {
          ...
          if(kinData[CH].motionData[index_step[CH]].Cn<=240) kinData[CH].motionData[index_step[CH]].Cn=240;
          kinData[CH].motionData[index_step[CH]].dir = kinData[CH].motionData[index_step[CH]-1].dir;
        }
      }
      else
...
int MKVelProfile::gen_circl_profile( CIRCLEProfile & circleProfile)
{
  ...
  for (int n = 0; n < circleProfile.arcAng; n++)
  {
    ...
    //% it to next step...
      ...
      sumDist[CH] = sumDist[CH] + kinData[CH].motionData[index_step[CH]].steps*STEP2DIST[CH];
      index_step[CH]++;
      if(index_step[CH]>=MAX_MOTIONDATA_SIZE-1) return ERROR_SIZE;
    }// --------- for(CH)
  }
    for(int CH=0; CH<3; CH++) {
...
// Handling single Z axis motion...
    ...
    double  curZPos=0;
    int nAxis=3;
    if(fabs(spiralProfile.heightZ)>=0.01) nAxis = 4;
    /////////////////////////////////////////////////////////////////////////
    // Calculation for index 0
